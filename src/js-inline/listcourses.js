...
const listcourses = {
  ...
  clickTrashCan(fetchedCourses, fetchedRounds) {
    ...
    function clickTrash(event) {
      ...
      if (fetchedRounds == null) { // there aren't as the response form the db was null
        ...
        if(Object.keys(clickedCourseObj ).length > 0) {
          ...
          listcourses.modalChoices(target, coursesModal, clickedCourseObj, clickedCourseID, fetchedCourses, fetchedRounds);
    ...
    courseList.addEventListener('click', clickTrash);

  }, // end clickTrashCan

  modalChoices(targetDOM, coursesModal, clickedCourseObj, clickedCourseID, fetchedCourses, fetchedRounds) {
    ...
    function clickedButtonListener(event) {
      ...
      buttonClicked = event.target.closest('.list-courses-modal-choices');
  ...
  deleteCourse(target, clickedCourseID, fetchedCourses) {
    ...
    if (document.querySelector('.list-courses-modal-open')) {
      document.querySelector('.list-courses-modal').classList.toggle('list-courses-modal-open');
